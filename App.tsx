...
const App: React.FC = () => {
  ...
  const handleSliderChange = (setter: (v: number) => void) => (value: number) => {
    ...
    if (!hasTrackedInteraction.current) {
      ...
      trackPixelCustomEvent('CalculatorInteraction');
    }
  };
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
        ...
        <div className="space-y-8 w-full max-w-md mx-auto lg:max-w-none">
          ...
          <section className="bg-white p-6 sm:p-8 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 relative overflow-hidden">
            ...
            <RangeInput label="Valor da Sessão" value={sessionPrice} min={50} max={600} step={10} unit="R$ " onChange={handleSliderChange(setSessionPrice)} colorClass="text-emerald-600" />
            <RangeInput label="Capacidade Semanal (Vagas)" value={weeklyCapacity} min={1} max={40} onChange={handleSliderChange(setWeeklyCapacity)} colorClass="text-slate-900" />
            <RangeInput label="Pacientes Atuais" value={currentPatients} min={0} max={weeklyCapacity} onChange={handleSliderChange(setCurrentPatients)} colorClass="text-slate-600" />
